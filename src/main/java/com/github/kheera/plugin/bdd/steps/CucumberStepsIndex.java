package com.github.kheera.plugin.bdd.steps;

import com.github.kheera.plugin.bdd.BDDFrameworkType;
import com.github.kheera.plugin.bdd.CucumberJvmExtensionPoint;
import com.github.kheera.plugin.bdd.OptionalStepDefinitionExtensionPoint;
...
public class CucumberStepsIndex {
    ...
    private static final String TEST_STEP_ANNOTATION_CLASS = "com.kheera.annotations.TestStep";
    private static final String TEST_MODULE_ANNOTATION_CLASS = "com.kheera.annotations.TestModule";

    private static final String TEST_MODULE_FEATURE_FILE_ATTRIBUTE = "featureFile";
     ...
     */
    public PsiFile createStepDefinitionFile(
        ...
        return new JavaStepDefinitionCreator().createStepDefinitionContainer(featureFile, dir, fileNameWithoutExtension);
    }
     ...
     * More info is available in {@link #findStepDefinitions(com.intellij.psi.PsiFile, GherkinStep)} doc
     *
     * @param featureFile file with steps
     * @param step        step itself
     * @return definition or null if not found
     * @see #findStepDefinitions(com.intellij.psi.PsiFile, GherkinStep)
     */
    @Nullable
    ...
    public PsiClass getStepDefinitionContainerPsiClass(@NotNull final PsiFile featureFile) {
        ...
        for (PsiClass moduleClass : moduleClasses) {
            PsiModifierList list = moduleClass.getModifierList();
            PsiAnnotation[] annotations = list.getAnnotations();
            for(PsiAnnotation ann : annotations) {
                if(ann.getQualifiedName().equals(TEST_MODULE_ANNOTATION_CLASS)) {
                    PsiAnnotationMemberValue m = ann.findAttributeValue(TEST_MODULE_FEATURE_FILE_ATTRIBUTE);
                    String propValue = m.getText();
